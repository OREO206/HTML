...
const asc = function(x, y) {
    return x - y; // 오름차순으로 정렬하려면 앞 - 뒤
}
array.sort(asc);
console.log(array); // 오름차순

array.sort(function(x, y) {return y - x});
console.log(array); // 내림차순
...
console.log(objcects);

objcects.sort(function(a, b) {
    return a.x - b.x;
});
console.log(objcects);
...
/*
    ...
    - 원본 배열 변경 x
    - braek, continue,... 부가적인 작업을 추가 x
    - for 문 보다 성능 떨어짐, 가독성이 좋고 편리하므로 자주 사용됨
...
/*
    ...
    - 원본 배열 변경 X
    - forEach 단순히 요소를 가지로 작업만 진행 (반환 값X), map 요소를 가지고 작업한 결과로 배열 새로 만듦 (반환 값 O)
*/
...
/*
    filter(콜백 함수) : 콜백함수의 반환 값이 true인 배열의 요소 값만을 추출한 새로운 배열을 생성
...
*/
const findIndexResut = array.findIndex(function(item, index) {
    return item % 2 === 1;
});
console.log(findIndexResut);
